...
Usage:
  ...
  projects/ops-agent/scripts/kosmos-auto.sh watch <fns|bank|suppliers|livesklad|manual>
...
Commands:
  ...
  watch      Запускает приёмщик новых загрузок из Downloads по выбранному источнику.
...
cmd_inbox() {
  ...
  for source_kind in "${source_names[@]}"; do
    ...
    printf "%-10s documents=%s agent=%s\n" "$source_kind" "$(count_files "$human_dir")" "$(count_files "$tech_dir")"
  done
}
